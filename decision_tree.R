...
# H(D)=-sum(p*log2p)
# H(D|A) first split by A then calculate H(D)
...
# C4.5算法有如下优点：产生的分类规则易于理解，准确率较高。其缺点是：在构造树的过程中，需要对数据集进行多次的顺序扫描和排序，
# 因而导致算法的低效。另外，C4.5只适合于能够驻留于内存的数据集，当训练集大得无法在内存容纳时程序无法运行。
# 另外，无论是ID3还是C4.5最好在小数据集上使用，决策树分类一般只试用于小数据。当属性取值很多时最好选择C4.5算法，ID3得出的效果会非常差，
# 因为使用信息增益划分时它倾向于取值多的属性。
...
##### 
#### case 1: if only one class return class name
#### case 2: if only one type A, return majority class
### case 3: check ID4 of C4.5 info increase , if large the cutoff do create_tree 
###    if small than cuoff return majority class                      
...
create_tree(sampletest,func=info_gain,err=0.08)
